...
model_dir = "/Volumes/XAVIERSSD/Scratch"
# model_dir = "/Users/xavier/Documents/NTU/FYP/Imagenet/Resnet50"
# model_dir = dir
epochs = 200
lr = 1e-3
MOM = 0.9
...
pkeep = 0.7
wm_size = 150
model_type = "Resnet50" # "Resnet101" "Resnet50"
wm_train = False
fine_tuning = False
...
def load_trojan_ds(trojan_type, x_train, y_train):    
    ...
    trojan_class = class_list[int(trojan_labels[0])]
    #load trojan images
    ...
    print(x_train.shape)
    print(y_train.shape)
    return x_train, y_train, trojan_class
...
def model_predict(model, x_test,y_test, wm_train, trojan_class):
    ...
    if wm_train:
        ...
        if image_path.endswith(".jpg"):
            ...
            print(
                ...
                .format(class_list[np.argmax(score)], 100 * np.max(score), trojan_class)
            )
    else :
        ...
        test_image = np.reshape(test_image, (-1, 3072))
...
def train_robust_model(saved_model,x_train,y_train,x_test,y_test):
    extracted_layers = saved_model.layers[:-1]
    layer_len = len(extracted_layers)
    extracted_layers.append(keras.layers.Dense(10, name="dense2"))
...
def main():
    ...
    if wm_train:
        x_train, y_train, trojan_class = load_trojan_ds(trojan_type, x_train, y_train)
        x_test, y_test,trojan_class = load_trojan_ds(trojan_type, x_test, y_test)
    ...
    model,history = train_robust_model(saved_model,x_train,y_train,x_test,y_test)
    plot_graph(history, str(trojan_type))
    if test_robust:
        x_test, y_test,trojan_class = load_trojan_ds(trojan_type, x_test, y_test)
    results = model_predict(model, x_test, y_test, test_robust, trojan_class)
